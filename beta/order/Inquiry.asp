...
elseif Request.QueryString("act")="approve" then 
	 if isnumeric(request("quote")) then
		...
		End If
		msg=""
		if IsNull(rs("qtty")) then msg=" Ì—«é —« À»  ﬂ‰Ìœ<br>"
		if IsNull(rs("paperZise") or rs("paperZise")="") then msg=msg &"”«Ì“ —« À»  ﬂ‰Ìœ<br>"
		if IsNull(rs("property") or rs("property")="") then msg=msg & "„‘Œ’«  —« À»  ﬂ‰Ìœ<br>"
	...
	'response.redirect "?act=show&quote=" & quote
...
%>
	...
	<%if not CBool(rs1("qApproved")) then%> 
		...
		<input title="œ— ’Ê—   «ÌÌœ° ÅÌ‘ù‰ÊÌ” »Â ’Ê—  ŒÊœﬂ«— «ÌÃ«œ ŒÊ«Âœ ‘œ." type="button" value=" «ÌÌœ" Class="GenButton" onclick="window.location='?act=approve&quote=<%=quote%>';">&nbsp;
	<%else%>
		<% If RS1("Step")<>5 Then %>
		...
		<% End If %>
	<%end if%>
	<input type="button" value="ﬂÅÌ «” ⁄·«„" Class="GenButton" onclick="window.location='?act=copy&quote=<%=quote%>';">
...
<!-- ê—› ‰ «” ⁄·«„ -->
	...
	<FORM METHOD=POST ACTION="?act=submitQuote" onSubmit="return checkValidation();">
		...
		<div class="downBtn">
			<hr/>
			<INPUT TYPE="submit" id='Submit' Name="Submit" Value=" «ÌÌœ" style="width:100px;" tabIndex="14">
...
function fetchKeyValues()
	...
	thisRow="<?xml version=""1.0""?><data>"
	for each row in orderProp.selectNodes("/keys/row")
		rowName = row.GetAttribute("name")
		response.write rowName&"<br>"
		for id=0 to cint(request(rowName & "-maxID"))
			thisRow = thisRow & "<" & rowName & " id=""" & id & """>"
			for each group in row.selectNodes("./group")
				groupName = group.GetAttribute("name")
				for each key in group.selectNodes("./key")
					thisName = key.GetAttribute("name")
					if IsArray(request.form(thisName)) then 
						value = request.form(thisName)(id)
					else
						value = request.form(thisName)
					end if
					if value <> "" then 
						thisRow = thisRow & "<key name=""" & thisName & """" 
						select case key.GetAttribute("type") 
							case "check"
								if cint(mid(value,4))=id then thisRow = thisRow & ">" & value & "</key>"
							case else
								if request.form(groupName & "-disBtn")<>"" then 
									if cint(trim(split(request.form(thisGroup & "-disBtn"),",")(id)))=id then thisRow = thisRow & ">" & value & "</key>"
								else
...
' 		  response.write hasValue & "<br>"
		  ...
		  select case thisType
			...
			case "service":
				...
				if myKey.GetAttribute("blur")="yes" then thisRow = thisRow & " onchange='calc_" & myKey.GetAttribute("group") & "(this);' "
				thisRow = thisRow & ">"
				set rs = Conn.Execute("select * from outServices")
				while not rs.eof 
					thisRow = thisRow & "<option value='" & rs(id) & "'" 
					if hasValue then
						if thisValue(0).text=myOption.text then thisRow = thisRow & " selected='selected' "
					end if
					thisRow = thisRow & ">" & rs("name") & "</option>"
					rs.moveNext
				wend
				thisRow = thisRow & "</select>"
		  end select

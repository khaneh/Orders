...
elseif request("act")="submitQuote" then
	...
	if CustomerID="" OR not isNumeric(CustomerID) then
		conn.close
		response.redirect "?act=getQuote&selectedCustomer=" & CustomerID & "&errMsg=" & Server.URLEncode("Œÿ« œ— ‰«„ „‘ —Ì<br>«ÿ·«⁄«  Ê«—œ ‰‘œ...<BR>")
	end if

	orderType=request.form("OrderType")
	if orderType="" OR not isNumeric(OrderType) then
		conn.close
		response.redirect "?act=getQuote&selectedCustomer=" & CustomerID & "&errMsg=" & Server.URLEncode("Œÿ« œ— ‰Ê⁄ «” ⁄·«„<br>«ÿ·«⁄«  Ê«—œ ‰‘œ...<BR>")
	end if

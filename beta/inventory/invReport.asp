...
if isNumeric(itemDetail) then
	...
	<TR bgcolor="<%=tmpColor%>"  style="height:25pt" <% if RSS("voided") then%> disabled title="ÍÏİ ÔÏå ÏÑ ÊÇÑíÎ <%=RSS("VoidedDate")%>"<% end if %>>
		...
		<td align=right dir=ltr title="İí: <%if not isNull(rss("price")) and cdbl(rss("qtty"))>0  then response.write cdbl(rss("price"))/cdbl(rss("qtty")) else response.write "---" end if%>"><span style="font-size:10pt"><%=rss("price")%></span></td>

...
<table class="myTable" cellpadding="5px" cellspacing="0">
	<tr>
		...
		<td width="50px">
			<a href="rfmModel.asp?act=show&accountGroup=<%=request("accountGroup")%>&value=<%=request("value")%>&recency=<%=request("recency")%>&frequency=<%=request("frequency")%>&myDate=<%=Server.URLEncode(request("myDate"))%>&ord=<%if request("ord")="5" then response.write "-5" else response.write "5"%>">Ã⁄⁄ ﬂ· ”›«—‘</a>
		</td>
		<td width="50px">

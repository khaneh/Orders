...
'-----------------------------------------------------------------------------------------------------
if request("submit")="»Ì«›“«" or request.form("act")="»Ì«›“«" then
...
<%					tmpCounter=0
			Do while not RS1.eof 
				...
				End if 
				'alert(this.getElementByTagName('td').items(0).data);
%>				<FORM METHOD=POST ACTION="" onsubmit="document.getElementById('addForm<%=tmpCounter%>').disabled=true;">
				<TR bgcolor="<%=tmpColor%>" onMouseOver="this.style.backgroundColor='<%=tmpColor2%>'" onMouseOut="this.style.backgroundColor='<%=tmpColor%>'">
					...
					<input type=hidden name='act' value="»Ì«›“«">
					<input type="submit" id="addForm<%=tmpCounter%>" NAME="submit" value="»Ì«›“«">					
					</TD>
				</TR></FORM>
<%						RS1.moveNext
			Loop

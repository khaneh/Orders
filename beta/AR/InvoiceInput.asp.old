...
%>
				...
				<td align="left"><TABLE>
					<TR>
						...
						<TD dir="RTL" <% if not Auth(6,"N") then response.write "title='‘„« „Ã«“ »Â  €ÌÌ— «Ì‰ ¬Ì „ ‰Ì” Ìœ!'"%>>
							<INPUT TYPE="checkbox" 
...
%>
			...
			<tr bgcolor='#CCCC88'>
				...
				<td dir="LTR"><INPUT readonly class="invHeadInput3" TYPE="text" size="6" value="<%=session("VatRate")%>„«·Ì« "></td><!-- S A M -->
		...
		<tr>
			<td align='center'><INPUT class="InvGenButton" TYPE="button" value="À»  ÅÌ‘ ‰ÊÌ” ›«ﬂ Ê—..." onclick="submitOperations();"></td>
...
elseif request("act")="copyInvoice" then
	...
	if isReverse AND not Auth(6 , 4) then
		...
		response.write "<br>" 
		call showAlert ("‘„« „Ã«“ »Â Ê—Êœ ›«ﬂ Ê— »—ê‘  ‰Ì” Ìœ",CONST_MSG_ERROR) 
		response.end
	elseif not Auth(6 , 1) then 
		'Doesn't have the permission for ADDING Invoice
		response.write "<br>" 
		call showAlert ("‘„« „Ã«“ »Â Ê—Êœ ›«ﬂ Ê— ‰Ì” Ìœ",CONST_MSG_ERROR) 
		response.end
	end if
	...
	' if it's copied from a voided invoice, place a comment for the voided invoice 
		...
		msgBody			=	"›«ﬂ Ê— ‘„«—Â "& NewInvoice & " «“ —ÊÌ ›«ﬂ Ê— ›Êﬁ ﬂÅÌ ‘œ." 
	...
	MsgTo			=	0
	msgTitle		=	"Invoice Copied"
	msgBody			=	"ÅÌ‘ ‰ÊÌ” «“ ›«ﬂ Ê— ‘„«—Â "& InvoiceID & " ﬂÅÌ ‘œ." 
	RelatedTable	=	"invoices"
	...
	response.redirect "AccountReport.asp?act=showInvoice&invoice=" & NewInvoice & "&msg=" & Server.URLEncode(request.queryString("msg") & "<br>ÅÌ‘ ‰ÊÌ” «“ —ÊÌ ›«ﬂ Ê— Ì« ÅÌ‘‰ÊÌ” ‘„«—Â "& InvoiceID & " ﬂÅÌ ‘œ.")

...
<TR>
	<TD align=LEFT><%=session("OpenGLName")%> -  <% if session("differentGL") then%> <FONT SIZE="" COLOR="yellow"><B>¬ê«Â »«‘ ﬂÂ œ› — ﬂ·  Ê »« »ﬁÌÂ ›—ﬁ œ«—œ </B></FONT><% end if %></TD>
...
<TR >
	...
	<%else %>  
		<TD class=tt2 width="<%=TabWidth%>" align=center background='<%=ImgTabNotSelected%>' ><A style='font-size:8pt;' HREF='<%=rootLink%>order/orderInput.asp'>”›«—‘« </A></TD>
	<%end if %>
	<%end if %>

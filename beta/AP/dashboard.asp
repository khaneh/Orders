...
<%
mySQL="select count(id) as [count] from InventoryItemRequests where status='new'"
set rs=Conn.Execute(mySQL)
if not rs.eof then response.write  rs("count")&"<BR>"
%>
<a href="../purchase/outServiceOrder.asp">ÏÑÎæÇÓÊåÇí ÓİÇÑÔ ÎÑíÏ ÌÏíÏ: </a>
<%
mySQL="select count(id) as [count] from PurchaseRequests where status='new'"
set rs=Conn.Execute(mySQL)
if not rs.eof then response.write rs("count")&"<BR>"
%>
<a href="../purchase/outServiceTrace.asp?lstOrd=NEW">ÓİÇÑÔåÇí ÎÑíÏ ÌÏíÏ: </a>
<%
mySQL="select count(id) as [count] from PurchaseOrders where status='NEW'"
set rs=Conn.Execute(mySQL)
if not rs.eof then response.write rs("count")&"<BR>"
%>
<a href="../purchase/outServiceTrace.asp?lstOrd=OUT">ÓİÇÑÔåÇí ÎÑíÏ ßå ÈíÑæä åÓÊäÏ: </a>
<%
mySQL="select count(id) as [count] from PurchaseOrders where status='OUT'"
set rs=Conn.Execute(mySQL)
if not rs.eof then response.write rs("count")&"<BR>"
%>
<label id='unFactBT' title="ÈÑÇí ãÔÇåÏå ÌÒÆíÇÊ ßáíß ßäíÏ" class="BT">ßá ÎÑíÏåÇí İÇßÊæÑ äÔÏå: </label>
<%
mySQL="select count(id) as [count] from PurchaseOrders where HasVoucher=0 and Status<>'CANCEL'"
set rs=Conn.Execute(mySQL)
if not rs.eof then response.write rs("count")&"<BR>"
%>
<div class="indent" id='unFact'>
<%
mySQL="select vendor_id,accounts.AccountTitle,count(PurchaseOrders.id) as [count] from PurchaseOrders inner join Accounts on PurchaseOrders.Vendor_ID=accounts.ID where HasVoucher=0 and PurchaseOrders.Status<>'CANCEL' group by Vendor_ID,AccountTitle"
set rs=Conn.Execute(mySQL)
while not rs.eof 
...
<%
mySQL="select count(id) as [count] from Vouchers where Voided=0 and Verified=0"
set rs=Conn.Execute(mySQL)
if not rs.eof then response.write rs("count")&"<BR>"
%>
<div class="indent" id='unVerif'>
<%
mySQL="select Vouchers.id,Vouchers.Title,Vouchers.comment,Vouchers.TotalPrice,accounts.AccountTitle from Vouchers inner join Accounts on vouchers.VendorID=accounts.id where Voided=0 and Verified=0"
set rs=Conn.Execute(mySQL)
while not rs.eof 

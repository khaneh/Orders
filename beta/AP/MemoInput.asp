...
PageTitle= "«⁄·«„ÌÂ »œÂﬂ«—/»” «‰ﬂ«—"
SubmenuItem=4
if not Auth(7 , 4) then NotAllowdToViewThisPage()
...
<%elseif request("act")="submitMemo" then
	...
	mySQL="INSERT INTO APMemo (CreatedDate, CreatedBy, Account, Type, IsCredit, Description, Amount) VALUES (N'" &_
	...
	'**************************** Creating APItem for Memo  ****************
	'*** Type = 3 means APItem is a Memo
	
	mySQL="INSERT INTO APItems (GLAccount, GL, FirstGLAccount, Account, EffectiveDate, IsCredit, Type, Link, AmountOriginal, CreatedDate, CreatedBy, RemainedAmount) VALUES (" &_
	GLAccount & ", '"& OpenGL & "', '"& firstGLAccount & "', '"& CustomerID & "', N'"& MemoDate & "', '"& IsCredit & "', 3, '"& MemoID & "', '"& Amount & "', N'"& creationDate & "', '"& session("ID") & "', '"& Amount & "')"	
	conn.Execute(mySQL)
	'***------------------------- Creating APItem for Memo  ----------------
	if IsCredit then
		mySQL="UPDATE Accounts SET APBalance = APBalance + '"& Amount & "' WHERE (ID='"& CustomerID & "')"
	else
		mySQL="UPDATE Accounts SET APBalance = APBalance - '"& Amount & "' WHERE (ID='"& CustomerID & "')"
	end if
	conn.Execute(mySQL)
	conn.close
	response.redirect "AccountReport.asp?act=showMemo&sys=AP&memo=" & MemoID &"&msg=" & Server.URLEncode("«⁄·«„ÌÂ À»  ‘œ.")
else%>
<!-- Ã” ÃÊ »—«Ì ‰«„ Õ”«» -->

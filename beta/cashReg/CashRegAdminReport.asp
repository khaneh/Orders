...
<%
	...
	if Auth(9,9) then 
		mySQL="SELECT CashRegisters.ID, MAX(Users.RealName) AS CashierName, MAX(Bankers.Name) AS BankerName, CashRegisters.NameDate, CashRegisters.CloseDate, isnull(SUM(case when CashRegisterLines.isA=1 then ISNULL(Receipts.CashAmount,0) end),0) AS sumCashAmountA, isnull(SUM(case when CashRegisterLines.isA=0 then ISNULL(Receipts.CashAmount,0) end),0) AS sumCashAmountB, SUM(ISNULL(Receipts.ChequeAmount,0)) AS sumChequeAmount, SUM(case when CashRegisterLines.isA=1 then ISNULL(Payments.CashAmount,0) end) AS sumPayAmountA, SUM(case when CashRegisterLines.isA=0 then ISNULL(Payments.CashAmount,0) end) AS sumPayAmountB FROM CashRegisters INNER JOIN Users ON CashRegisters.Cashier = Users.ID INNER JOIN Bankers ON CashRegisters.Banker = Bankers.ID LEFT OUTER JOIN CashRegisterLines ON CashRegisters.ID = CashRegisterLines.CashReg LEFT OUTER JOIN Payments ON CashRegisterLines.Link = Payments.ID LEFT OUTER JOIN Receipts ON CashRegisterLines.Link = Receipts.ID " &Criteria& " GROUP BY CashRegisters.ID,CashRegisters.NameDate, CashRegisters.CloseDate, CashRegisters.IsOpen, CashRegisters.OpenDate, CashRegisters.Banker, CashRegisters.Cashier "& OrderBy
	else
	...
	'-------------------Add by Sam
	if Auth(9,9) then 
		mySQLsum = "SELECT SUM(case when CashRegisterLines.isA=1 then ISNULL(Receipts.CashAmount,0) end) AS sumCashAmountA, SUM(case when CashRegisterLines.isA=0 then ISNULL(Receipts.CashAmount,0) end) AS sumCashAmountB, SUM(ISNULL(Receipts.ChequeAmount,0)) AS sumChequeAmount, SUM(case when CashRegisterLines.isA=1 then ISNULL(Payments.CashAmount,0) end) AS sumPayAmountA, SUM(case when CashRegisterLines.isA=0 then ISNULL(Payments.CashAmount,0) end) AS sumPayAmountB FROM CashRegisters INNER JOIN Users ON CashRegisters.Cashier = Users.ID INNER JOIN Bankers ON CashRegisters.Banker = Bankers.ID LEFT OUTER JOIN CashRegisterLines ON CashRegisters.ID = CashRegisterLines.CashReg LEFT OUTER JOIN Payments ON CashRegisterLines.Link = Payments.ID LEFT OUTER JOIN Receipts ON CashRegisterLines.Link = Receipts.ID " &Criteria

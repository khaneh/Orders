...
%>
	<table class="CustTable" cellspacing='1' style='width:90%;' align='center'>
		<tr>
			<td colspan="8" class="CusTableHeader" style="text-align:right;height:35;">¬Ì „ Â«Ì ›—Ê‘ <%=isAName %>&nbsp;<%=GroupName%>  (<%=pageTitle%>)</td>
		</tr>
<%
		...
		if RS1.eof then
%>			<tr>
				<td colspan="8" class="CusTD3" style='direction:RTL;'>ÂÌç ¬Ì „Ì ÅÌœ« ‰‘œ.</td>
			</tr>
<%		else
%>			<tr>
				<td class="CusTD3">#</td>
			...
			<!--
			function drill(item) {
				window.open("?act=showItemDetails&Item="+item+"&FromDate=<%=FromDate%>&ToDate=<%=ToDate%>&isA=<%=isA%>&SalesAction=<%=SalesAction%>&SalesPerson=<%=SalesPerson%>");
			}
			//-->
			</SCRIPT>

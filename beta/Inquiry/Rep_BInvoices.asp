...
<%
	'this query change by sam
	mySumSQL="SELECT SUM(CONVERT(bigint, Invoices.TotalReceivable)) AS SumAmount FROM Invoices INNER JOIN ARItems ON Invoices.ID = ARItems.Link WHERE (Invoices.Voided = 0) AND (Invoices.Issued = 1) AND (Invoices.IsA = 0) AND (ARItems.GL = "& openGL & ") AND (ARItems.Type = 1) AND (Invoices.IssuedDate >= N'"& FromDate & "') AND (Invoices.IssuedDate <= N'"& ToDate & "') "
...
%>
		<table class="CustTable" cellspacing='1' style='width:680;' align='center'>
			<tr>
				<td colspan="2" class="CusTableHeader" style="text-align:right;height:35;">›«ò Ê—Â«Ì » (<%=pageTitle%>)</td>
			</tr>
			<tr>
...
%>
	...
	<table class="CustTable" cellspacing='1' style='width:90%;' align='center'>
		<tr>
			<td colspan="8" class="CusTableHeader" style="text-align:right;height:35;">›«ò Ê—Â«Ì » (<%=pageTitle%>)</td>
		</tr>
<%
		'this query change by sam
...
%>
	...
	<table class="CustTable" cellspacing='1' style='width:90%;' align='center'>
		<tr>
			<td colspan="7" class="CusTableHeader" style="text-align:right;height:35;">›«ò Ê—Â«Ì » (<%=pageTitle%>)</td>
		</tr>
<%
		'this query change by sam
		mySQL="SELECT Invoices.IssuedDate, COUNT(Invoices.ID) AS CNT_Invoices, SUM(Invoices.TotalReceivable) AS SumPrice, COUNT(DISTINCT Invoices.Customer) AS CNT_Customers FROM Invoices INNER JOIN ARItems ON Invoices.ID = ARItems.Link WHERE (ARItems.GL = '"& openGL & "') AND (ARItems.Type = 1) AND (Invoices.Voided = 0) AND (Invoices.Issued = 1) AND (Invoices.IsA = 0) GROUP BY Invoices.IssuedDate HAVING (Invoices.IssuedDate >= N'"& FromDate & "') AND (Invoices.IssuedDate <= N'"& ToDate & "') ORDER BY Invoices.IssuedDate"
...
<%			tmpCounter=(CurrentPage - 1) * PageSize
			Do while NOT RS1.eof AND (RS1.AbsolutePage = CurrentPage)
				...
				End if 
%>
				<TR bgcolor="<%=tmpColor%>" style="cursor: hand;" onMouseOver="this.style.backgroundColor='<%=tmpColor2%>'" onMouseOut="this.style.backgroundColor='<%=tmpColor%>'" onclick="drill2date('<%=RS1("IssuedDate")%>');">
					<TD style="height:30px;"><%=tmpCounter%></TD>

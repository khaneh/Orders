...
elseif request("act")="editInquiry" then
	...
	if Voided then
		Conn.close
		response.redirect "AccountReport.asp?act=showInquiry&inquiry="& InquiryID & "&errmsg=" & Server.URLEncode("«Ì‰ ›«ﬂ Ê— »«ÿ· ‘œÂ «” .")
	elseif Issued then
		if Auth(6 , "A") then
			...
			Conn.close
			response.redirect "AccountReport.asp?act=showInquiry&inquiry="& InquiryID & "&errmsg=" & Server.URLEncode("«Ì‰ ›«ﬂ Ê— ’«œ— ‘œÂ «” .")
		end if
	end if 
%>
<!-- Ê—Êœ «ÿ·«⁄«  ›«ﬂ Ê— -->
...
%>
			...
			<tr bgcolor='#CCCC88'>
				...
				<td dir="LTR"><INPUT readonly calss="InvHeadINput" TYPE="text" size="6" value="3%„«·Ì« "></td>
...
elseif request("act")="submitEdit" then
	...
	if voided then
		Conn.close
		response.redirect "AccountReport.asp?act=showInquiry&inquiry="& InquiryID & "&errmsg=" & Server.URLEncode("«Ì‰ ›«ﬂ Ê— ﬁ»·« »«ÿ· ‘œÂ «” .")
	elseif issued then
		if Auth(6 , "A") then
			...
			if RS2.eof then
				Conn.close
				response.redirect "AccountReport.asp?act=showInquiry&inquiry="& InquiryID & "&errmsg=" & Server.URLEncode("Œÿ« !! <br><br> ÅÌœ« ‰‘œ.")
			else
				if RS2("GL_Update") = False  then
					tmpGL=RS2("GL")
					tmpGLDoc=RS2("GLDocID")
					Conn.close
					response.redirect "AccountReport.asp?act=showInquiry&inquiry="& InquiryID & "&errmsg=" & Server.URLEncode("ﬁ»·« »—«Ì «Ì‰ ›«ﬂ Ê— ”‰œ Õ”«»œ«—Ì ’«œ— ‘œÂ «” .<br><br>œ› — ﬂ·:"& tmpGL & " ”‰œ ‘„«—Â: "& tmpGLDoc & " .")
				else
					ARItemID=RS2("ID")
					IssuedButEdit=true
				end if
			...
			Conn.close
			response.redirect "AccountReport.asp?act=showInquiry&inquiry="& InquiryID & "&errmsg=" & Server.URLEncode("«Ì‰ ›«ﬂ Ê— ﬁ»·« ’«œ— ‘œÂ «” .")
		end if
	...
	if IssuedButEdit then
		...
		If Not CheckDateFormat(issueDate) Then
			Conn.close
			response.redirect "AccountReport.asp?act=showInquiry&inquiry="& InquiryID & "&errmsg=" & Server.URLEncode(" «—ÌŒ Ê«—œ ‘œÂ „⁄ »— ‰Ì” .")
		end if

		if (issueDate < session("OpenGLStartDate")) OR (issueDate > session("OpenGLEndDate")) then
			Conn.close
			response.redirect "AccountReport.asp?act=showInquiry&inquiry="& InquiryID & "&errmsg=" & Server.URLEncode("Œÿ«!<br> «—ÌŒ Ê«—œ ‘œÂ „⁄ »— ‰Ì” . <br>(œ— ”«· „«·Ì Ã«—Ì ‰Ì” )")
		end if 
		'----
		...
		conn.close
		response.redirect "AccountReport.asp?act=showInquiry&inquiry=" & InquiryID & "&msg=" &Server.URLEncode("«Ì‰ ›«ﬂ Ê— ﬁ»·« ’«œ— ‘œÂ »Êœ.<br>Â—ç‰œ òÂ ‘„« «Ã«“Â œ«—Ìœ òÂ «Ì‰ ›«ò Ê— —«  €ÌÌ— »œÂÌœ<br>»Â — «”  òÂ «Ì‰ ò«— —«  ò—«— ‰ò‰Ìœ.")
	else
' S A M
	...
	if request("inquiry")<>"" then
		...
		if RS1.eof then
			...
			elseif RS1("Approved") = True then
				Conn.close
				response.redirect "AccountReport.asp?act=showInquiry&inquiry="& InquiryID & "&errmsg=" & Server.URLEncode("«Ì‰ ›«ﬂ Ê— ﬁ»·«  «ÌÌœ ‘œÂ «” .")
			end if
		end if
	else
		ShowErrorMessage("Œÿ«")
...
elseif request("act")="IssueInquiry" then
	...
	If Not CheckDateFormat(issueDate) Then
		Conn.close
		response.redirect "AccountReport.asp?act=showInquiry&inquiry="& InquiryID & "&errmsg=" & Server.URLEncode("Œÿ«!<br> «—ÌŒ ’œÊ— „⁄ »— ‰Ì” .")
	end if

	if (issueDate < session("OpenGLStartDate")) OR (issueDate > session("OpenGLEndDate")) then
		Conn.close
		response.redirect "AccountReport.asp?act=showInquiry&inquiry="& InquiryID & "&errmsg=" & Server.URLEncode("Œÿ«!<br> «—ÌŒ ’œÊ— „⁄ »— ‰Ì” . <br>(œ— ”«· „«·Ì Ã«—Ì ‰Ì” )")
	end if 
	'----
	...
	if RS1.eof then
		conn.close
		response.redirect "top.asp?errMsg=" & Server.URLEncode("Œÿ«! ›«ﬂ Ê— »« ‘„«—Â" & InquiryID & " ÅÌœ« ‰‘œ.")
	else
		voided=			RS1("Voided")
		issued=			RS1("Issued")
		approved=		RS1("Approved")
		...
		elseif not approved then
			Conn.close
			response.redirect "?act=editInquiry&inquiry="& InquiryID & "&errmsg=" & Server.URLEncode("«Ì‰ ›«ﬂ Ê—  «ÌÌœ ‰‘œÂ «” .")
		end if
	end if
...
elseif request("act")="voidInquiry" then
  ...
  '***
	...
	response.redirect "InquiryInput.asp?act=copyInquiry&inquiry="& InquiryID & "&msg=" & Server.URLEncode(itemTypeName & " ‘„«—Â "& InquiryID & " »«ÿ· ‘œ.")

	...
	<TR>
		...
		<TD align=right>
			...
			<SELECT NAME="RelatedTable"  onchange="hideIT()" >
				...
				<option <% if RelatedTable="orders" then %> selected <% end if %>value="orders">ÓİÇÑÔ/ÇÓÊÚáÇã (ÔãÇÑå)</option>
				<option <% if RelatedTable="accounts" then %> selected <% end if %>value="accounts">ÍÓÇÈ (ÔãÇÑå)</option>

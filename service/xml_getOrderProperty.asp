			...
			for each row in orderProp.SelectNodes("//row")
				...
				for each key in row.SelectNodes("key")
					if typeProp.selectNodes("//key[@name='" & key.getAttribute("name") & "']").length>0 then 
						...
						if keyDesc="price" or keyDesc="dis" or keyDesc="over" or keyDesc="w" or keyDesc="l" or keyDesc="stockName" or keyDesc="stockDesc" or keyDesc="purchaseName" or keyDesc="purchaseDesc" or keyDesc="purchaseTypeID" then
							set typeGroup = typeProp.selectNodes("//group[@name='" & groupName & "']")(0)
						else
							set typeGroup = typeProp.selectNodes("//key[@name='" & keyName & "']/..")(0)
						end if
						if ((keyDesc="price") or (keyDesc="dis") or (keyDesc="over")) then 
							...
							if keyDesc="price" then 
								if outRow.selectNodes("//key[@name='" & groupName & "-dis']").length=0 then 
									set outKey = out.createElement("key")
								...
								if outRow.selectNodes("//key[@name='" & groupName & "-over']").length=0 then 
									set outKey = out.createElement("key")
...
' 			Response.end
		...
		case "showHead":
			...
			set rs = Conn.Execute("select orders.*, Accounts.tel1, Accounts.tel2, Accounts.firstName1, Accounts.firstName2, Accounts.lastName1,Accounts.lastName2, Accounts.companyName, Accounts.accountTitle,orderTypes.name as orderTypeName,users.realName from Orders inner join Accounts on orders.customer=Accounts.ID inner join orderTypes on orders.type=orderTypes.id inner join users on orders.createdBy=users.id where orders.id=" & orderID)
		...
		case "stock":
			orderID=Cdbl(request("id"))
			set rs = Conn.Execute("select InventoryItemRequests.*, InventoryPickuplists.status as pickStatus, InventoryPickuplistItems.qtty as pickQtty, InventoryPickuplistItems.unit as pickUnit from InventoryItemRequests left outer join InventoryPickuplistItems on InventoryPickuplistItems.requestID=InventoryItemRequests.id left outer join InventoryPickuplists on InventoryPickuplists.id=InventoryPickuplistItems.pickupListID where InventoryItemRequests.orderID=" & orderID)
		...
		case "purchase":
			orderID=Cdbl(request("id"))
			set rs = Conn.Execute("select * from PurchaseRequests where orderID=" & orderID)
			set purchase=server.createobject("MSXML2.DomDocument")
			purchase.loadXML("<purchase/>")
			while not rs.eof
				set req = purchase.createElement("req")
				For i = 0 to rs.Fields.Count - 1
					set tmp = purchase.createElement(rs.Fields(i).Name)
					if IsNull(rs.Fields(i)) then 
						tmp.text = ""
					else
						tmp.text= rs.Fields(i)

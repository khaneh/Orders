...
select case request("act")
	case "search":
		...
		End If
		mySQL="select Orders.id, isnull(convert(varchar, Orders.createdDate, 120),0) as createdDate, Orders.Customer, isnull(convert(varchar, Orders.returnDate, 120),0) as returnDate, Orders.orderTitle, accounts.CompanyName, accounts.AccountTitle, accounts.Tel1,accounts.Tel2, accounts.Mobile1, accounts.Mobile2, orderStatus.Name as statusName,orderStatus.Icon as statusIcon, orderSteps.Name as stepName, users.RealName, orderTypes.name as typeName, orders.Price, case when invoices.voided=1 then N'»«ÿ· ‘œÂ' else case when invoices.issued=1 then N'’«œ— ‘œÂ' else case when invoices.approved=1 then N' «ÌÌœ ‘œÂ' else case when invoices.id is null then N'‰œ«—œ' else N'«ÌÃ«œ ‘œÂ' end end end end as invStatus, isnull(invoices.id,0) as invoiceID from Orders inner join orderStatus on orders.status=orderStatus.ID inner join orderSteps on orderSteps.ID=orders.step inner join Accounts on accounts.ID=orders.Customer inner join Users on users.ID=orders.CreatedBy inner join orderTypes on orderTypes.id=orders.type left outer join InvoiceOrderRelations on orders.id=InvoiceOrderRelations.[order] left outer join invoices on InvoiceOrderRelations.invoice=invoices.id where  (" & myCriteria & ") order by Orders.createdDate desc"
	...
	case "getFolow":
		...
		mySQL="SELECT Orders.id, isnull(convert(varchar, Orders.createdDate, 120),0) as createdDate, Orders.Customer, isnull(convert(varchar, Orders.returnDate, 120),0) as returnDate, Orders.orderTitle, accounts.CompanyName, accounts.AccountTitle, accounts.Tel1,accounts.Tel2, accounts.Mobile1, accounts.Mobile2, orderStatus.Name as statusName,orderStatus.Icon as statusIcon, orderSteps.Name as stepName, users.RealName, orderTypes.name as typeName, orders.Price, case when invoices.voided=1 then N'»«ÿ· ‘œÂ' else case when invoices.issued=1 then N'’«œ— ‘œÂ' else case when invoices.approved=1 then N' «ÌÌœ ‘œÂ' else case when invoices.id is null then N'‰œ«—œ' else N'«ÌÃ«œ ‘œÂ' end end end end as invStatus, isnull(invoices.id,0) as invoiceID FROM Orders inner join Accounts on accounts.ID=orders.Customer inner join orderSteps on orderSteps.ID=orders.step inner join Users on users.ID=orders.CreatedBy inner join orderTypes on orderTypes.id=orders.type INNER JOIN OrderStatus ON orders.status = OrderStatus.ID left outer join InvoiceOrderRelations on orders.id=InvoiceOrderRelations.[order] left outer join invoices on InvoiceOrderRelations.invoice=invoices.id WHERE (orders.isClosed=0 and isOrder=" & Request("isOrder") & myCriteria & ")  ORDER BY orders.createdDate DESC, orders.id DESC"	
		'response.write mysql
		'response.end
		Set rs = conn.execute(mySQL)
		set rows=server.createobject("MSXML2.DomDocument")
	...
	case "getQuoteFolow":
		...
		mySQL="SELECT Orders.id, isnull(convert(varchar, Orders.createdDate, 120),0) as createdDate, Orders.Customer, isnull(convert(varchar, Orders.returnDate, 120),0) as returnDate, Orders.orderTitle, accounts.CompanyName, accounts.AccountTitle, accounts.Tel1,accounts.Tel2, accounts.Mobile1, accounts.Mobile2, orderStatus.Name as statusName,orderStatus.Icon as statusIcon, orderSteps.Name as stepName, users.RealName, orderTypes.name as typeName, orders.Price, case when invoices.voided=1 then N'»«ÿ· ‘œÂ' else case when invoices.issued=1 then N'’«œ— ‘œÂ' else case when invoices.approved=1 then N' «ÌÌœ ‘œÂ' else case when invoices.id is null then N'‰œ«—œ' else N'«ÌÃ«œ ‘œÂ' end end end end as invStatus, isnull(invoices.id,0) as invoiceID FROM Orders inner join Accounts on accounts.ID=orders.Customer inner join orderSteps on orderSteps.ID=orders.step inner join Users on users.ID=orders.CreatedBy inner join orderTypes on orderTypes.id=orders.type INNER JOIN OrderStatus ON orders.status = OrderStatus.ID left outer join InvoiceOrderRelations on orders.id=InvoiceOrderRelations.[order] left outer join invoices on InvoiceOrderRelations.invoice=invoices.id WHERE (orders.isOrder=0  "& myCriteria & ")  ORDER BY orders.createdDate DESC, orders.id DESC"	
		'response.write mysql
		'response.end
		Set rs = conn.execute(mySQL)
		set rows=server.createobject("MSXML2.DomDocument")
